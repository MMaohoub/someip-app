...
class Transmitter {
 public:
  Transmitter(bool _use_tcp, uint32_t _cycle)
      : app_(vsomeip::runtime::get()->create_application()),
        ...
        running_(true),
        is_offered_(false),
        offer_thread_(std::bind(&Transmitter::run, this)),
        notify_thread_(std::bind(&Transmitter::notify, this)) {}
  ...
  bool init() {
    ...
    app_->register_state_handler(
        std::bind(&Transmitter::on_state, this, std::placeholders::_1));

    app_->register_message_handler(
        SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_GET_METHOD_ID,
        std::bind(&Transmitter::on_get, this, std::placeholders::_1));

    app_->register_message_handler(
        SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_SET_METHOD_ID,
        std::bind(&Transmitter::on_set, this, std::placeholders::_1));
...
Transmitter *its_sample_ptr(nullptr);
...
int main(int argc, char **argv) {
  ...
  Transmitter tx - signal - adapter(use_tcp, cycle);

  its_sample_ptr = &tx - signal - adapter;
  signal(SIGINT, handle_signal);
  signal(SIGTERM, handle_signal);

  if (tx - signal - adapter.init()) {
    tx - signal - adapter.start();
    return 0;
  } else {
    return 1;
  }
